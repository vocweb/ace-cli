...
# ACE CLI Coding Harness Roadmap

## Goal

Turn ace-cli into the most **automatable** coding harness:
...
5. Surface `doctor` / preflight diagnostics in onboarding docs and help — **done**: README + USAGE now put `ace doctor` / `/doctor` in the first-run path and point at the built-in preflight report
...
8. Promote `doctor` from slash-only to top-level CLI entrypoint — **done**: `ace doctor` is now a local shell entrypoint with regression coverage for direct help and health-report output
9. Make machine-readable status commands actually machine-readable — **done**: `ace --output-format json status` and `ace --output-format json sandbox` now emit structured JSON snapshots instead of prose tables
...
17. **Orphaned module integration audit** — **done**: `runtime` now keeps `session_control` and `trust_resolver` behind `#[cfg(test)]` until they are wired into a real non-test execution path, so normal builds no longer advertise dead automation surface area.
...
19. **Subcommand help falls through into runtime/API path** — **done**: `ace doctor --help`, `ace status --help`, `ace sandbox --help`, and nested `mcp`/`skills` help are now intercepted locally without runtime/provider startup, with regression tests covering the direct CLI paths.
...
26. **Resumed local-command JSON parity gap** — **done**: direct `ace --output-format json` already had structured renderers for `sandbox`, `mcp`, `skills`, `version`, and `init`, but resumed `claw --output-format json --resume <session> /…` paths still fell back to prose because resumed slash dispatch only emitted JSON for `/status`. Resumed `/sandbox`, `/mcp`, `/skills`, `/version`, and `/init` now reuse the same JSON envelopes as their direct CLI counterparts, with regression coverage in `rust/crates/rusty-claude-cli/tests/resume_slash_commands.rs` and `rust/crates/rusty-claude-cli/tests/output_format_contract.rs`.
...
We should consider ace-cli materially more automatable when:
...
`worker_boot.rs` has a solid `WorkerStatus` state machine (`Spawning → TrustRequired → ReadyForPrompt → Running → Finished/Failed`). It is exported from `runtime/src/lib.rs` as a public API. But ace-cli is a **plugin** loaded inside the `opencode` binary — it cannot add HTTP routes to `opencode serve`. The HTTP server is 100% owned by the upstream opencode process (v1.3.15).
...
**Recommended path:** Option 3 — emit `WorkerStatus` transitions to `.claw/worker-state.json` on every state change. This is purely within ace-cli's plugin scope, requires no upstream changes, and gives clawhip a file it can poll to distinguish a truly stalled worker from a quiet-but-progressing one.

**Action item:** Wire `WorkerRegistry::transition()` to atomically write `.claw/worker-state.json` on every state transition. Add a `ace state` CLI subcommand that reads and prints this file. Add regression test.
...
**Decision:** `ace state` reading `.claw/worker-state.json` is the **blessed observability contract** for clawhip and downstream tooling. This is not a stepping-stone — it is the supported surface. Build against it.

**Rationale:**
- ace-cli is a plugin running inside the opencode binary. It cannot add HTTP routes to `opencode serve` — that server belongs to upstream sst/opencode.
...
- `ace state --output-format json` gives clawhip everything it needs: `status`, `is_ready`, `seconds_since_update`, `trust_gate_cleared`, `last_event`, `updated_at`.
...
1. After `WorkerCreate`, poll `.claw/worker-state.json` (or run `ace state --output-format json`) in the worker's CWD at whatever interval makes sense (e.g. 5s).
