...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    if let Ok(ace_config_home) = std::env::var("ACE_CONFIG_HOME") {
        push_prefixed_skill_lookup_roots(&mut roots, std::path::Path::new(&ace_config_home));
    }
    ...
    push_skill_lookup_root(
        &mut roots,
        std::path::PathBuf::from("/home/bellman/.ace/skills"),
        SkillLookupOrigin::SkillsDir,
    );
    push_skill_lookup_root(
        &mut roots,
...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".ace"));
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".ace"));
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".ace").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("ACE_CONFIG_HOME") {
        ...
        .map_err(|_| {
            String::from(
                "HOME is not set (on Windows, set USERPROFILE or HOME, \
                 or use ACE_CONFIG_HOME to point directly at the config directory)",
            )
        })?;
    Ok(PathBuf::from(home).join(".ace"))
}
...
mod tests {
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        use std::fs;
        // Write a .ace/settings.json in a temp dir with trustedRoots
        let worktree = temp_path("config-trust-worktree");
        let ace_dir = worktree.join(".ace");
        ...
        fs::write(ace_dir.join("settings.json"), settings).expect("write settings");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .ace/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".ace").join("worker-state.json");
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".ace").join("skills").join("plan");
        let command_dir = root.join(".ace").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".ace/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".ace/commands/handoff.md"));
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".ace")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".ace")).expect("cwd dir");
        std::fs::write(
            home.join(".ace").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        let local_settings = std::fs::read_to_string(cwd.join(".ace").join("settings.local.json"))
            .expect("local settings after enter");
        assert!(local_settings.contains(r#""defaultMode": "plan""#));
        let state =
            std::fs::read_to_string(cwd.join(".ace").join("tool-state").join("plan-mode.json"))
